...
export const checkApiConfig = async (): Promise<ApiConfigStatus> => {
  try {
    ...
    // 无论是否在Desktop模式，都尝试获取Settings
    let settings
    try {
      settings = await settingsApi.getSettings()
      console.log('获取SettingsSuccess:', settings)
    } catch (error) {
      console.warn('获取SettingsFailed，可能不在Desktop模式:', error)
      // 如果获取SettingsFailed，检查是否在Desktop模式
      if (!isDesktop) {
        console.log('非Desktop模式，返回无配置Status')
        return {
      ...
      // Desktop模式下获取SettingsFailed，也返回无配置
      console.log('Desktop模式下获取SettingsFailed，返回无配置Status')
      return {
        hasValidConfig: false,
        missingProviders: ['LLM API'],
        currentProvider: undefined,
    ...
    if (!settings || !settings.api || !settings.api.api_keys) {
      console.log('Settings数据不完整:', { settings, hasApi: !!settings?.api, hasApiKeys: !!settings?.api?.api_keys })
      return {
        hasValidConfig: false,
        missingProviders: ['LLM API'],
        currentProvider: undefined,
    ...
    // 修复：api_model 是模型Name，不是提供商Name
    // 需要从 api_provider 或 llm_provider 获取提供商Name
    const currentProvider = settings.api.api_provider || settings.api.llm_provider || 'dashscope'
    
    console.log('API配置Details:', {
      currentProvider,
      apiKeys: {
    ...
    console.error('检查API配置Failed:', error)
    return {
      hasValidConfig: false,
      missingProviders: ['LLM API'],
      currentProvider: undefined,
...
export const showApiConfigModal = (missingProviders: string[], onNavigateToSettings?: () => void) => {
  ...
  Modal.confirm({
    ...
    content: (
      <div style={{ color: '#fff' }}>
        <p style={{ color: '#fff', marginBottom: '12px', fontSize: '14px' }}>创建Project需要配置以下API:</p>
        <p style={{ fontWeight: 'bold', color: '#40a9ff', marginBottom: '12px', fontSize: '16px' }}>{missingNames}</p>
        <p style={{ color: '#f0f0f0', fontSize: '14px' }}>请前往Settings页面进行配置.</p>
      </div>
    ),
    okText: '去配置',
    cancelText: 'Cancel',
...
export const validateApiConfigBeforeProjectCreation = async (): Promise<boolean> => {
  ...
  console.log('API配置有效，允许创建Project')
  return true
}

/**
 * 获取API配置Status的友好Description
 */
