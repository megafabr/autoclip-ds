...
export const SimpleProgressDemo: React.FC = () => {
  ...
  const handleStartProcessing = (projectId: string) => {
    ...
    message.success(`开始处理Project: ${projectId}`)
  }

  // 模拟查看Details
  const handleViewDetails = (projectId: string) => {
    message.info(`查看ProjectDetails: ${projectId}`)
  }
  ...
  const handleRetry = (projectId: string) => {
    ...
    message.success(`重试Project: ${projectId}`)
  }

  // Add新Project
  const handleAddProject = () => {
    if (!newProjectId.trim()) {
      message.warning('请输入ProjectID')
      return
    }
    ...
    const newProject = {
      ...
      description: '这是一个新Add的演示Project',
    ...
    setNewProjectId('')
    message.success(`AddProject: ${newProjectId}`)
  }

  // 开始轮询选中的Project
  const handleStartPolling = () => {
    if (selectedProjectIds.length === 0) {
      message.warning('请选择要轮询的Project')
      return
    }
    startPolling(selectedProjectIds, pollingInterval)
    message.success(`开始轮询 ${selectedProjectIds.length} 个Project`)
  }
