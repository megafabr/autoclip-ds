...
const BilibiliDownload: React.FC<BilibiliDownloadProps> = ({ onDownloadSuccess }) => {
  ...
  const parseVideoInfo = async () => {
    ...
    setParsing(true)
    setError('') // Clear previous error information
    ...
    try {
      ...
      setError('') // Parsing succeeded, clear error information
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (task.status === 'completed') {
          ...
          setPollingInterval(null)
          setDownloading(false)
          message.error(`Download failed: ${task.error_message || 'UnknownError'}`)
          resetForm()
        }
      } catch (error) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (response.project_id) {
        ...
        message.success(`${platformName}Project created successfully. Downloading in background, you can continue adding projects`)
  ...
  const stopDownload = () => {
    ...
    setDownloading(false)
    setCurrentTask(null)
    message.info('Download task monitoring stopped')
  }
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '16px' }}>
        <Space direction="vertical" style={{ width: '100%' }} size={16}>
          <div>
            <Input.TextArea
              placeholder="Paste Bilibili or YouTube video link. Supported:&#10;• Bilibili：https://www.bilibili.com/video/BV1xx411c7mu&#10;• YouTube：https://www.youtube.com/watch?v=xxxxx"
              value={url}
              onChange={(e) => {
          ...
          {videoInfo && (
            <>
              ...
              <div>
                ...
                <Select
                  placeholder="Select browser to get cookie (optional)"
